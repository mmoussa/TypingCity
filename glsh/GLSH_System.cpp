...
void Window::_postEvent(const InputEvent& e)
{
    mEventQueue.push_back(e);
}

void Window::_dispatchEvents()
{
    if (!mApp.isQuitting()) {
        for (auto& e : mEventQueue) {
            ...
            case MOUSE_DOWN_EVENT:
                mApp.onMouseDown(e.mbe);
                break;
            case MOUSE_UP_EVENT:
                mApp.onMouseUp(e.mbe);
                break;
            case MOUSE_MOTION_EVENT:
                mApp.onMouseMotion(e.mme);
                break;
            case MOUSE_SCROLL_EVENT:
                mApp.onMouseScroll(e.mse);
                break;
            default:
                // oops?
                break;
...
void System::Run(App& app, const std::string& title, int width, int height)
{
    ...
    try {
        ...
        if (app.initialize(width, height)) {
            // reset time to ignore initialization time
            wnd->resetTime();
        } else {
            // initialization failed, so kill the app
            //app.quit();
...
void System::MouseButtonCallback(int button, int state, int x, int y)
{
    ...
    if (state == GLUT_DOWN) {
        ...
        e.type = MOUSE_DOWN_EVENT;
    } else if (state = GLUT_UP) {
        wnd->getMouse()._injectButtonUp(mb);
        e.type = MOUSE_UP_EVENT;
    }
...
void System::MouseMotionCallback(int x, int y)
{
    ...
    mouse._injectMousePos(x, y);
    
    InputEvent e;
    e.type = MOUSE_MOTION_EVENT;
    e.mme.dx = x - mouse.getX();
    e.mme.dy = y - mouse.getY();
    wnd->_postEvent(e);
}
